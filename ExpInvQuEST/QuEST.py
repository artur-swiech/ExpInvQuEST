...
class QuEst:
    ...
    def z_from_Z(self, Z):
        M = self.M_from_Z(Z)
        return Z * (self.r * M * self.gamma + np.sqrt((self.r * M)**2 * (self.gamma**2 - 1.0) + 1.0))
    ...
    def dZdLambda_from_Z(self, Z):
        ...
        _gamma2m1 = self.gamma**2 - 1.0
        _a = np.sqrt((self.r * M)**2 * gamma2m1 + 1.0)
        _b = self.r**2 * M * gamma2m1
        dzdZ = self.r * M * self.gamma + _a + Z * (r * dMdZ * self.gamma + (_b * dMdZ) / _a)
        dzdLambda = Z * (self.r * dMdLambda * self.gamma + (_b * dMdLambda) / _a)
    ...
    def Q(self):
        ...
        dzdLambda = Z[:, None] * (self.r * dMdLambda * self.gamma + self.r**2 * MZ[:, None] * dMdLambda * (self.gamma**2 - 1.0) / N[:, None])
        dzdZ = self.r * MZ * self.gamma + N + Z * (self.r * dMdZ * self.gamma + self.r**2 * MZ * dMdZ * (self.gamma**2 - 1.0) / N)
        ...
        dadLambdak2diff = 0.5 * np.array([((dadLambda[m+1:l]).T * k2diff[m+1:l]).sum(axis=1) for m,l in zip(M,L)])
        ...
        dQdLambda = np.where(M==L,
                - (amdxidLambda.T * xi) +
                0.5 * (dadLambdam.T * x2diff) +
                aldxip1dLambda.T * xip1
                             ,
                0.5 * (dadLambdam.T * (kmp1**2 - xi**2)) - amdxidLambda.T * xi +
                dadLambdak2diff.T +
                0.5 * (dadLambdal.T * (xip1**2 - kl**2)) + aldxip1dLambda.T * xip1
                            ).T

        Q = self.N * np.array(Q)
        dQdLambda = self.N * np.array(dQdLambda)
